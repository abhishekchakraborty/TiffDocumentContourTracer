...
create_file_if_needed(char out_name[],short **out_image,int length,int width)
{
	/*if(does_not_exist(out_name))
    {*/
	    create_allocate_tiff_file(out_name, length, width, out_image);
...
create_allocate_tiff_file(char file_name[],int image_length,
						  int image_width,short ** image)
{
...
*/
   ...
   insert_short_into_buffer(buffer, 8,image_width);
   bytes_written = fwrite(buffer, 1, 12, image_file);
   /*printf("\n wrote %d bytes", bytes_written);
*/
   ...
   insert_short_into_buffer(buffer, 2, 3);
   insert_short_into_buffer(buffer, 4, 1);
   insert_short_into_buffer(buffer, 8,image_length);
   bytes_written = fwrite(buffer, 1, 12, image_file);
   /*printf("\n wrote %d bytes", bytes_written);
*/
   ...
   insert_short_into_buffer(buffer, 2, 3);
   insert_short_into_buffer(buffer, 4, 1);
   insert_short_into_buffer(buffer, 8,8);
   bytes_written = fwrite(buffer, 1, 12, image_file);
   /*printf("\n wrote %d bytes", bytes_written);
*/
...
*/
   /*k = 1; image_header->image_length * image_header->image_width;
*/
...
*/
   ...
   write_array_into_tiff_image(file_name,image,
                         1 ,
                         1 ,
                         1 +image_length,
                         1 +image_width ,
...
*/
   ...
   position   = fseek(image_file,296,SEEK_SET);
   ...
   for(i=0; i<(ll-il); i++)
   {
	  ...
	  position      = seek_to_end_of_line(image_file, le, width);
